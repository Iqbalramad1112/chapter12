...
def home():
    ...
    try:
        ...
        return redirect(url_for('login', msg="There was an logging your in"))
...
def get_posts():
    ...
    try:
        ...
        if username_receive == "":
            # Fetch posts for the authenticated user if no specific username is provided
            posts = list(db.posts.find({"username": payload["id"]}).sort("date", -1).limit(20))
        else:
